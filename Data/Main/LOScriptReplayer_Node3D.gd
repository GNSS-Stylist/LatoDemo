...
@export var replayTimeOverride:float = 0
...
@export var loOverrideFraction:float = 0
@export var locationOverride:Vector3 = Vector3(0,0,0)
@export var orientationOverride:Quaternion = Quaternion(0,0,0,1)
...
#func _physics_process(delta):
	...
	if (replayTimeOverride != 0):
		...
		currentReplayTime = Global.replayTime_Lidar + replayTimeShift
	...
	var basisTemp:Basis = Basis(quat.slerp(orientationOverride, loOverrideFraction))
